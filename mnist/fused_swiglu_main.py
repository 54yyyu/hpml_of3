...
class Net(nn.Module):
    def __init__(self):
        ...
        # fused-branch SwiGLU: pack two (9216→2048) mats into one (9216→4096)
        self.Wf       = nn.Parameter(torch.randn(9216, 2 * 4096))
        # final 10-class head
    ...
    def forward(self, x):
        ...
        # fused Triton SwiGLU (two-branch GEMM + gate)
        B = x.size(0)
        D = 4096
        block = 64
        # allocate output tensor [B, D]
        Z = torch.empty(B, D, device=x.device)
